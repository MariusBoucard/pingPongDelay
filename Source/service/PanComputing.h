...
private :
    ...
    int moduloMesure = 2;
public:
      void setModuloMesure(std::string modulo){
        if(modulo == "1/1"){
            moduloMesure = 4;
        } 
        if (modulo == "1/2")
    ...
    float computePan(ParametersPan parameters)
    {
        ...
        else
        {
            ...
            delta = fmod(delta, 1.0f);
            if(delta<lastDelta){
                renduMesure = (renduMesure +1)%moduloMesure;
            }
            ...
            if(parameters.pingPongNote == "1/1"){
                moduloMesure =4;
                delta = delta / 4;
                delta+=renduMesure*0.25f;
            }

            if (parameters.pingPongNote == "1/2")
            {
                // TODO : Keep Record of last mesure
                // TODO : Change modulo mesure From outside
                moduloMesure =2;
                delta = delta / 2;
                delta+=renduMesure*0.5f;
